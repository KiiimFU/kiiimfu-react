...
export default function Header({ onSuggest }) {
  ...
  <header className="fixed top-0 left-0 right-0 z-50 bg-[#8dcff9] text-white shadow-md h-16 flex items-center px-6">
    ...
    <div className="flex items-center gap-3 flex-shrink-0">
      ...
      <button
          onClick={onSuggest}
          className="hidden sm:flex px-4 py-2 rounded-lg font-medium bg-[#8dcff9] hover:bg-white/20 transition-colors cursor-pointer">
          💡 Suggest
      </button>
      ...
      {menuOpen && (
        <div className="absolute top-16 left-0 right-0 bg-[#8dcff9] flex flex-col shadow-md sm:hidden">
          ...
          <button
            onClick={() => { setMenuOpen(false); onSuggest() }}
            className="px-8 py-3 border-t border-white/20 font-medium hover:bg-white/20 text-left cursor-pointer"
          >
            💡 Suggest
          </button>
        </div>
